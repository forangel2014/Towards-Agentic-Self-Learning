...
class vLLMRollout(BaseRollout):
    def __init__(self, model_path: str, config: DictConfig, tokenizer, model_hf_config, **kwargs):
        ...
        pipeline_parallel_size = self.config.get("pipeline_model_parallel_size", 1)
        if pipeline_parallel_size > 1:
            raise ValueError("vLLM SPMD does not support pipeline parallelism yet, set rollout.mode=async")
        ...
        self.recompute_mm = kwargs.get("recompute_mm", False)
        ...
        if config.get("limit_images", None):  # support for multi-image data
            mm_kwargs["limit_mm_per_prompt"] = {"image": config.get("limit_images")}
            # NOTE(wuhuan): 没必要加，dataloader 处理了就行, 否则可能会和processor.json中的 pixels 不一致
            # if config.image_resolution:
            #     mm_processor_kwargs = {"max_pixels": config.image_resolution * config.image_resolution}
            #     mm_kwargs["mm_processor_kwargs"] = mm_processor_kwargs
            # mm_kwargs["disable_mm_preprocessor_cache"] = True,  # 暂时不开，会导致 AGI vllm 挂

        logger.info(f"[VLLM] {mm_kwargs=}")

        self.is_multimodal = limit_mm_per_prompt is not None
        ...
        kwargs = dict(
            n=1,
            logprobs=1 if self.config.calculate_log_probs else 0,  # can be set to 0 and let actor to recompute
    ...
    def profiler(self):
        if os.getenv("VLLM_TORCH_PROFILER_DIR"):
            self.inference_engine.start_profile()
        try:
            yield
        finally:
            if os.getenv("VLLM_TORCH_PROFILER_DIR"):
                self.inference_engine.stop_profile()
    ...
    def generate_sequences(self, prompts: DataProto, **kwargs) -> DataProto:
        ...
        if "multi_modal_data" in non_tensor_batch and self.is_multimodal:
            vllm_inputs = []
            for raw_prompt_ids, multi_modal_data in zip(
                non_tensor_batch.pop("raw_prompt_ids"), non_tensor_batch.get("multi_modal_data", None), strict=True
            ):
                vllm_inputs.append({"prompt_token_ids": raw_prompt_ids, "multi_modal_data": multi_modal_data})

            # NOTE(wuhuan): 如果不重计算 mm inputs，则无需透传 multi_modal_data
            # if not self.recompute_mm:
        ...
        for input_data in vllm_inputs:
            ...
            if not input_data.get("multi_modal_data", {}).get("image"):
                input_data.pop("multi_modal_data", None)
        ...
        if rollout_n := prompts.meta_info.get("n"):
            kwargs["n"] = rollout_n
        ...
        if self.config.agent.activate_agent:
            ...
            meta_info.update(agent_proto.meta_info or {})
            info_once(f" [DEBUG agent output proto] {batch.keys()=}, {non_tensor_batch.keys()=}")

        if "multi_modal_inputs" in non_tensor_batch.keys() and self.recompute_mm:
            non_tensor_batch.pop("multi_modal_inputs")
        if "origin_multi_modal_data" in non_tensor_batch.keys():
            non_tensor_batch.pop("origin_multi_modal_data")

        # NOTE(wuhuan): 这两个字段在后面算 点点 reward 以及 multi session 时候要用到，而且size不大，直接透传
        # if "raw_prompt" in non_tensor_batch.keys():
        #     non_tensor_batch.pop("raw_prompt")
        # if "multi_modal_data" in non_tensor_batch.keys() and not self.recompute_mm:
        #     non_tensor_batch.pop("multi_modal_data")
        ...
        info_once(f"{prompts.batch.batch_size=} rollout {batch.batch_size=}")
...
class vLLMAsyncRollout:
    ...
    def load_model(self, *args, **kwargs):
        ...
        # NOTE(wuhuan): 第一个 step 不会触发 wakeup, load dummy 时需要手动触发，否则权重是随机的
