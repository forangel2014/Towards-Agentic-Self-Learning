...
class ActorRolloutRefWorker(Worker, DistProfilerExtension):
    ...
    def _build_model_optimizer(
        ...
        with init_context(), warnings.catch_warnings():
            ...
            if use_liger:
                ...
                logger.info(f"Applied Liger kernel to {role} module: {actor_module.__class__.__name__}")
        ...
        # NOTE(wuhuan): save memory
        if not self._is_actor:
            actor_module.requires_grad_(False)

        if self.config.model.freeze_vision_tower:
            vision_names = [
                "visual",  # qwen2vl
                "vision_model",  # <= agivlm1.6
                "vision_tower",  # agivlm1.7
            ]
            for vision_name in vision_names:
                vision_tower = getattr(actor_module, vision_name, None)
        ...
        if role == "actor" and optim_config is not None:
            ...
            actor_optimizer = optim.AdamW(
                ...
                # NOTE(wuhuan): fused for saving memory
                fused=True,
            )
    ...
    def _build_rollout(self, trust_remote_code=False):
        ...
        if rollout_name == "hf":
            ...
            rollout = vllm_rollout_cls(
                ...
                trust_remote_code=trust_remote_code,
                recompute_mm=self.config.model.get("recompute_mm", False),
                **lora_kwargs,
            )
    ...
    def init_model(self):
        ...
        if self._is_actor:
            ...
            self.actor = DataParallelPPOActor(
                ...
                recompute_mm=self.config.model.recompute_mm,
            )
    ...
    def generate_sequences(self, prompts: DataProto):
        ...
        with self.rollout_sharding_manager:
            ...
            if "timing" in output.meta_info and isinstance(output.meta_info["timing"], dict):
                timing_generate.update({f"generate_sequences/{k}": v for k, v in output.meta_info["timing"].items()})
...
class CriticWorker(Worker, DistProfilerExtension):
    ...
    def _build_critic_model_optimizer(self, config):
        ...
        if config.model.freeze_vision_tower and isinstance(critic_module, AutoModelForCausalLMWithValueHead):
            vision_names = [
                "visual",  # qwen2vl
                "vision_model",  # <= agivlm1.6
                "vision_tower",  # agivlm1.7
            ...
            for vision_name in vision_names:
                vision_tower = getattr(critic_module.pretrained_model, vision_name, None)
                if vision_tower is not None:
                    vision_tower.requires_grad_(False)
                    logger.info("Vision tower is set to not trainable.")
                    break
    ...
    def _build_model(self, config):
        ...
        if config.strategy == "fsdp":
            reward_module = FSDP(
                reward_module,
                param_init_fn=init_fn,
                use_orig_params=self.config.model.fsdp_config.get("use_orig_params", False),
                auto_wrap_policy=auto_wrap_policy,
                device_id=get_device_id(),
                sharding_strategy=sharding_strategy,  # zero3
                sync_module_states=True,
