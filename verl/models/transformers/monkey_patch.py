...
from .dummy_image import model_forward as qwen25vl_model_forward_with_dummy_image
...
def apply_monkey_patch_for_dummy_image(processor, module):
    ...
    if is_agivlm1_6(processor):
        ...
        print("Monkey patch AgiVlm1.6 dummy image for Zero-3 Hang bugs")
    elif is_agivlm1_7(processor):
        res = processor.image_processor(images=dummy_images, return_tensors="pt")
        unwrapped_module._dummy_mm_inputs = res
        print("Monkey patch AgiVlm1.7 dummy image for Zero-3 Hang bugs")
    elif isinstance(processor, Qwen2_5_VLProcessor):
        res = processor.image_processor(images=dummy_images, return_tensors="pt")
        unwrapped_module.model._dummy_mm_inputs = res
        Qwen2_5_VLModel.forward = qwen25vl_model_forward_with_dummy_image
        print("Monkey patch Qwen2.5vl dummy image for Zero-3 Hang bugs")
