...
class DataProto:
    ...
    def select(
        self, batch_keys=None, non_tensor_batch_keys=None, meta_info_keys=None, deepcopy=False, strict=True
    ) -> "DataProto":
    ...
    def padding(self, padding_size, padding_candidate="last"):
        ...
        if padding_size == 0:
            return
        if padding_candidate == "random":
            # 每次padding都随机选择一个不同的元素
            padding_parts = []
            for _ in range(padding_size):
                random_idx = random.randint(0, len(self) - 1)
                padding_parts.append(self.select_idxs([random_idx]))
            padding_part = DataProto.concat(padding_parts)
        elif padding_candidate == "first":
            padding_candidate = self.select_idxs([0])
            padding_part = padding_candidate.repeat(padding_size)
        elif padding_candidate == "last":
            padding_candidate = self.select_idxs([len(self) - 1])
            padding_part = padding_candidate.repeat(padding_size)
        else:
            raise ValueError(f"Unsupported padding_candidate: {padding_candidate}")
    ...
    def concat(data: list["DataProto"]) -> "DataProto":
        ...
        if "concat_meta_info_fn" in data[0].meta_info and data[0].meta_info["concat_meta_info_fn"]:
            fn = data[0].meta_info["concat_meta_info_fn"]
            data[0].meta_info = fn(data)
