...
class Tracking:
    ...
    def __init__(
        ...
        if "tensorboard" in default_backend:
            ...
            tensorboard_dir = config["trainer"].get("tensorboard_dir", None)
            if tensorboard_dir is None:
                tensorboard_dir = Path(config["trainer"]["default_local_dir"]) / "runs" / dtm
            else:
                tensorboard_dir = Path(tensorboard_dir) / dtm

            print(f"[TRACKING] {tensorboard_dir=}")
            self.logger["tensorboard"] = _TensorboardAdapter(tensorboard_dir)
            try:
                config_str = json.dumps(config, ensure_ascii=False)
            except BaseException as e:
                config_str = str(config)
                print(f"WARNING: {e} when dumping config to json, using str instead.")

            self.logger["tensorboard"].writer.add_text("runtime_config", config_str)
        ...
        if "clearml" in default_backend:
            self.logger["clearml"] = ClearMLLogger(
                full_project_name, experiment_name, config, continue_last_task=step > 0
            )

        if "webhook" in default_backend:
            self.logger["webhook"] = WebhookLogger(full_project_name, experiment_name, config, self.logger)

        if "rl_logging_board" in default_backend:
            from .logging_board import RLLoggingBoardLogger

            assert tokenizer is not None, "Please provide a tokenizer for rl_logging_board logger."
            rl_logging_board_dir = config["trainer"]["rl_logging_board_dir"]
            if not rl_logging_board_dir:
                rl_logging_board_dir = Path(config["trainer"]["default_local_dir"]) / "rl_logging_board"

            print(f"[TRACKING] {rl_logging_board_dir=}")
            self.logger["rl_logging_board"] = RLLoggingBoardLogger(
                str(rl_logging_board_dir), project_name, experiment_name, tokenizer
            )

    def log(self, data, step, backend=None, batch=None):
        for default_backend, logger_instance in self.logger.items():
            if backend is None or default_backend in backend:
                if default_backend == "rl_logging_board":
                    logger_instance.log(data=data, step=step, batch=batch)
                    continue
...
def run_git_command(cmds: list[str], directory_path):
    if not os.path.isdir(directory_path):
        return False, f"错误：目录 '{directory_path}' 不存在"

    git_dir = os.path.join(directory_path, ".git")
    if not os.path.isdir(git_dir):
        return False, f"错误：'{directory_path}' 不是一个Git仓库"

    try:
        result = subprocess.run(cmds, cwd=directory_path, capture_output=True, text=True, check=True)
        return True, result.stdout
    except subprocess.CalledProcessError as e:
        return False, f"Git命令执行错误: {e.stderr}"
...
class WebhookLogger:
    def __init__(self, project_name: str, experiment_name: str, config, loggers) -> None:
        ...
        self.webhooks = Configs.SINK_WEBHOOK.split(",") if Configs.SINK_WEBHOOK else []
        self._notify_when_start()
    ...
    def log(self, data, step):
        ...
        core_metric_fmt = {
            ...
            "actor/grad_norm": "{:.2f}",
            "actor/pg_loss": "{:.4f}",
            "actor/reward_kl_penalty": "{:.2f}",
            "actor/kl_loss": "{:.2f}",
            "actor/ppo_kl": "{:.5f}",
            "critic/score/mean": "{:.2f}",
            "response_length/mean": "{:.1f}",
            "agent/tool_call_mean": "{:.1f}",
        }
        core_metric_strs = [f"step:{step}"]
        for k in core_metric_fmt:
            if k in data:
...
"""
        ...
        if "clearml" in self.loggers:
            task_url = self.loggers["clearml"]._task.get_output_log_web_page()
            markdown += f"- 监控看板地址：[{task_url}]({task_url})\n"

        self._send_markdown(markdown + "\n")

    def _send_markdown(self, markdown: str):
        payload = {
            "msgtype": "markdown",
            "markdown": {
                "content": markdown,
...
class ClearMLLogger:
    def __init__(self, project_name: str, experiment_name: str, config, continue_last_task: bool = True):
        import os
        ...
        for k, v in data.items():
            if isinstance(v, int | float | np.floating | np.integer):
                ...
                if len(elements) <= 3:
                    ...
                    title = "/".join(elements[:2])
                    series = "/".join(elements[2:])
