    ...
    def remove_previous_save_local_path(self, path):
        ...
        for p in path:
            ...
            try:
                ...
                print(f"[warning] Checkpoint manager remove previous save local path failed, not found: {abs_path}")
...
def rotate_ckpt(local_dir: str, config, global_step: int):
    from loguru import logger

    ckpt_dirs = list(Path(local_dir).glob("global_step_*"))
    if not ckpt_dirs:
        return

    # +1 是因为要额外保存 latest，如果 latest 同时满足 save_freq ，下面会减掉
    max_ckpt = global_step // config.save_freq + 1
    ...
    ckpt_dirs = [(int(i.name.split("_")[-1]), i) for i in ckpt_dirs if i.name.split("_")[-1].isdigit()]
    if not ckpt_dirs:
        return
    ...
    if latest_ckpt[0] % config.save_freq == 0:
        max_ckpt -= 1

    logger.info(f"latest checkpoint: {latest_ckpt}")
    ckpt_num = 1
    for step, ckpt_dir in ckpt_dirs:
        if step % config.save_freq != 0 or ckpt_num >= max_ckpt:
            try:
