...
def load_mcore_dist_weights(parallel_model, dist_weight_path, is_value_model=False):
    ...
    for model in parallel_model:
        ...
        if isinstance(getattr(unwrap_model(model), "vision_model", None), PreTrainedModel):
            for k in list(ssd.keys()):
                if k.startswith("vision_model."):
                    ssd.pop(k)
            print("pop vision_model ssd keys")

            dist_checkpointing.load(
                ssd, dist_weight_path, strict=StrictHandling.IGNORE_ALL, validate_access_integrity=False
            )
        else:
