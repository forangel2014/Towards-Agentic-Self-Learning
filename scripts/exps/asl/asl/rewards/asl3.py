...
@rewards_registry.register(alias=["asl_reward_3"], ignore_if_exist=True)
class AgenticSelfLearning3Reward(WithWorkerGroupMixin, GRPORewards):
    ...
    def __call__(
        ...
        **kwargs,
        ...
        if do_meta_verification:
            ...
            if config.reward_model.online_prompt_generation:
                ...
                if "verify" in config.reward_model.pg_reward:
                    meta_gen_rm_scores = [0 for _ in range(len(meta_gen_rm_prompts))]
                    for i in range(sample_num):
                        meta_gen_rm_output_texts = get_policy_rm_output_batch(
                            config,
                            meta_gen_rm_prompts,
                            example_batch,
                            meta_answers,
                        ...
                        meta_gen_rm_scores_sample = compute_score(config, meta_gen_rm_output_texts)
                        meta_gen_rm_scores = [
                            meta_gen_rm_scores[i] + meta_gen_rm_scores_sample[i]
                    ...
                    logger.write(str(meta_gen_rm_scores) + "\n")
                    for i in range(len(meta_gen_rm_scores)):
                        if meta_gen_rm_scores[i] / sample_num < 0.8:
                            meta_scores[i] = 0
                ...
                if "binary" in config.reward_model.gen_rm_verification_method:
                    ...
                    for i in range(n_questions):
                        ...
                        minority_indices_local = [
                            j for j, score in enumerate(scores_this_question) if score == target_score
                        ]
                        majority_indices_local = [
                            j for j, score in enumerate(scores_this_question) if score != target_score
                        ]

                        if minority_indices_local and majority_indices_local:
                            # 从多数得分的索引中随机采样与少数得分索引相同数量的样本
                            sampled_majority_indices_local = random.sample(
                                majority_indices_local, len(minority_indices_local)
                            )
                    ...
                    verify_prompts = [
                        ...
                        for question_prompt, completion, solution in zip(
                            balanced_prompts, balanced_completions, verify_solutions, strict=False
                        )
                    ]

                else:
                    verify_solutions = [
                        f"conclusion: {['A', 'B', 'C', 'D', 'E'][int(1 - score * 5)]}"
                        for score in try_answering_scores
                    ]
                    verify_prompts = [
                        verify_prompt_template.format(
                            question_prompt=question_prompt, completion=completion, solution=solution
                        )
                        for question_prompt, completion, solution in zip(
                            question_prompts, try_answering_completions, verify_solutions, strict=False
                        )
                    ]

                logger.write("******************verify_prompts******************\n")
                logger.write(str(verify_prompts[0]) + "\n")
                logger.write("******************verify_solutions******************\n")
                logger.write(str(verify_solutions[0]) + "\n")
                meta_iter = save_new_qa_data(
                    config, verify_prompts, RETRIEVAL_SYS, style="rule", answers=verify_solutions
                )
                ...
                # meta_iter = save_new_qa_data(config, valid_questions, RETRIEVAL_SYS, style="gen_rm", answers=valid_answers, meta_iter_plus=True)
                meta_iter = save_new_qa_data(
                    config, valid_questions, RETRIEVAL_SYS, style="gen_rm", answers=valid_answers, meta_iter_plus=True
                )
                ...
                valid_questions = filter_and_duplicate_removal(filtered_meta_completions)
                meta_iter = save_new_qa_data(
                    config, valid_questions, RETRIEVAL_SYS, style="gen_rm", meta_iter_plus=True
                )
        ...
        if os.path.exists(f"./exp/{config.trainer.experiment_name}/meta_asl/qa_data_{meta_iter}.jsonl"):
            ...
            if config.reward_model.online_prompt_generation:
                if meta_iter % 3 == 1:
                    qa_data = load_qa_data(config, meta_iter + 1)
                    logger.write("******************last_dataset_len******************\n")
                    logger.write(str(last_dataset_len) + "\n")
                    if meta_iter == 1:
                        limit_size = 2000
                    else:
                        limit_size = 5000  # min(last_dataset_len, 20000) #* rollout_n * max(4 - meta_iter // 2, 1)
