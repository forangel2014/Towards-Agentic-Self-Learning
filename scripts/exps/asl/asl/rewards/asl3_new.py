...
You should use the tool to retrieve the relevant infomation about the question, then generate the new question by {question_constraint} one reasoning hop to the current question.
e.g. {demo}
The new question should be about factual knowledge and can be answered with ONLY ONE concreate entity.
Output with the following format:
<question>
...
3. The question must be solvable and the tool must be necessary to answer the question.
...
def build_gen_batch_with_prompts(config, prompts, completions, solutions, tokenizer, is_agentic, logger):
    ...
    verify_prompts = [
        verify_prompt_template.format(
            question_prompt=question_prompt.strip(), completion=completion.strip(), solution=solution.strip()
        )
...
def save_new_qa_data(config, questions, sys_prompt, style, answers=None, meta_iter_plus=False):
...
def get_question_quality(
    ...
    if len(success_questions):
        ...
        if config.reward_model.pg_type == "gen_rm":
        ...
        elif config.reward_model.pg_type == "rule":
            logger.write("******************question_output_texts******************\n")
            logger.write(question_output_texts[0] + "\n")
    ...
    try_answering_questions = []
    for question in success_questions:
        try_answering_questions.extend([question] * 10)

    return question_scores, try_answering_questions, success_answers, try_answering_completions, try_answering_scores
...
def prepare_meta_prompt(prompts, solutions, scores, is_agentic, logger, pg_types=("depth", "width")):
...
@rewards_registry.register(alias=["asl_reward_3_new"], ignore_if_exist=True)
class AgenticSelfLearning3RewardNew(WithWorkerGroupMixin, GRPORewards):
    ...
    def __call__(
        ...
        **kwargs,
        ...
        if hasattr(config.reward_model, "turns"):
            role_turns = config.reward_model.turns.split("-")
        else:
            role_turns = ["pg", "grm", "policy"]
        grm_turn = role_turns.index("grm") + 1
        pg_turn = role_turns.index("pg") + 1
        policy_turn = role_turns.index("policy") + 1

        logger.write("******************grm_turn******************\n")
        logger.write(str(grm_turn) + "\n")
        logger.write("******************pg_turn******************\n")
        logger.write(str(pg_turn) + "\n")
        logger.write("******************policy_turn******************\n")
        logger.write(str(policy_turn) + "\n")
        ...
        logger.write("******************meta_iter******************\n")
        logger.write(str(meta_iter) + "\n")

        if meta_iter <= 3 + pg_turn:
            pg_types = ["width"]
        else:
            pg_types = ["depth"]
        ...
        if do_gen_rm_verification:
            ...
            logger.write(gen_rm_output_texts[0] + "\n")
            logger.write("******************gen_rm_output_texts[-1]******************\n")
            logger.write(gen_rm_output_texts[-1] + "\n")
            ...
            if config.reward_model.online_prompt_generation:
                online_prompt_generation_new_prompts, answer_to_save = prepare_meta_prompt(
                    gen_rm_prompts, gen_rm_solutions, gen_rm_scores, is_agentic, logger, pg_types
                )
        ...
        if do_rule_verification:
            ...
            # if config.reward_model.online_prompt_generation:
            #     online_prompt_generation_new_prompts, answer_to_save = prepare_meta_prompt(rule_prompts, rule_solutions, rule_scores, is_agentic, logger)
            #     meta_iter = save_new_qa_data(config, online_prompt_generation_new_prompts, PROMPT_GENERATOR_SYS, style="meta", answers=answer_to_save)

        if do_meta_verification:
            # verify the quality of generated prompts, select the high quality prompts to save as new training data

            if config.reward_model.online_prompt_generation:
                meta_scores, question_prompts, success_answers, try_answering_completions, try_answering_scores = (
                    get_question_quality(
                        config,
                        example_batch,
                        meta_completions,
                        meta_answers,
                ...
                # meta_gen_rm_prompts = [f"<|im_start|>system\n{RETRIEVAL_SYS}<|im_end|>\n<|im_start|>user\n{meta_completions[i]}<|im_end|>\n<|im_start|>assistant\n" for i in range(len(meta_completions))]
                # if "verify" in config.reward_model.pg_reward:
                #     meta_gen_rm_output_texts = get_policy_rm_output_batch(config, meta_gen_rm_prompts, example_batch, meta_answers, meta_answers, tokenizer, rollout_model, is_agentic, rollout_n, logger)
                #     logger.write("******************meta_gen_rm_output_texts[0]******************\n")
                #     logger.write(meta_gen_rm_output_texts[0] + "\n")
                #     logger.write("******************meta_gen_rm_output_texts[-1]******************\n")
                #     logger.write(meta_gen_rm_output_texts[-1] + "\n")
                #     meta_gen_rm_scores = compute_score(config, meta_gen_rm_output_texts)
                #     logger.write("******************meta_gen_rm_scores******************\n")
                #     logger.write(str(np.mean(meta_gen_rm_scores)) + "\n")
                #     for i in range(len(meta_gen_rm_scores)):
                #         if meta_gen_rm_scores[i] == 0:
                #             meta_scores[i] = 0

                # n_questions = len(question_prompts) // 10
                # logger.write("******************len_question_prompts******************\n")
                # logger.write(str(len(question_prompts)) + "\n")
                # logger.write("******************n_questions******************\n")
                # logger.write(str(n_questions) + "\n")
                # balanced_prompts = []
                # balanced_answers = []
                # balanced_completions = []
                # balanced_scores = []
                # for i in range(n_questions):
                #     scores_this_question = try_answering_scores[i*10:(i+1)*10]
                #     if np.mean(scores_this_question) > 0.5:
                #         target_score = 0
                #     else:
                #         target_score = 1

                #     minority_indices_local = [j for j, score in enumerate(scores_this_question) if score == target_score]
                #     majority_indices_local = [j for j, score in enumerate(scores_this_question) if score != target_score]

                #     if minority_indices_local and majority_indices_local:
                #         # 从多数得分的索引中随机采样与少数得分索引相同数量的样本
                #         sampled_majority_indices_local = random.sample(majority_indices_local, len(minority_indices_local))
                #         balanced_indices_local = minority_indices_local + sampled_majority_indices_local
                #         offset = i * 10
                #         for local_idx in balanced_indices_local:
                #             global_idx = offset + local_idx
                #             balanced_prompts.append(question_prompts[global_idx])
                #             balanced_answers.append(success_answers[global_idx])
                #             balanced_completions.append(try_answering_completions[global_idx])
                #             balanced_scores.append(try_answering_scores[global_idx])
                ...
                # else:
                #     verify_prompt_template = THINKING_PROMPT_TEMPLATE
                # verify_solutions = ["correct" if score == 1.0 else "wrong" for score in balanced_scores]
                # #verify_prompts = [verify_prompt_template.format(question_prompt=question_prompt, completion=completion, solution=solution) for question_prompt, completion, solution in zip(question_prompts, try_answering_completions, verify_solutions)]
                # verify_prompts = [verify_prompt_template.format(question_prompt=question_prompt.strip(), completion=completion.strip(), solution=solution.strip()) for question_prompt, completion, solution in zip(balanced_prompts, balanced_completions, verify_solutions)]
                # logger.write("******************verify_prompts******************\n")
                # logger.write(str(verify_prompts[0]) + "\n")
                # meta_iter = save_new_qa_data(config, verify_prompts, RETRIEVAL_SYS, style="rule", answers=verify_solutions)
                ...
                if config.reward_model.pg_reward == "absolute_zero":
                    filtered_meta_completions = [
                        meta_completions[i] for i in range(len(meta_completions)) if meta_scores[i] > 0
                    ]
                    valid_questions = filter_and_duplicate_removal(filtered_meta_completions)
                ...
                # if "depth" in pg_types:
                #     logger.write("******************meta_scores[0]******************\n")
                #     logger.write(str(meta_scores[0]) + "\n")
                #     logger.write("******************meta_solutions[0]******************\n")
                #     logger.write(str(meta_solutions[0]) + "\n")
                #     logger.write("******************meta_completions[0]******************\n")
                #     logger.write(str(meta_completions[0]) + "\n")
                #     add_reasoning_hop_scores = compute_score_subem_verify(meta_completions, meta_solutions)
                #     for i in range(len(meta_completions)):
                #         add_reasoning_hop_scores[i] = 1 - add_reasoning_hop_scores[i]
                #         meta_scores[i] += add_reasoning_hop_scores[i] * (1-np.log(2))
                #     logger.write("******************add_reasoning_hop_scores[0]******************\n")
                #     logger.write(str(add_reasoning_hop_scores[0]) + "\n")
                #     logger.write("******************meta_scores[0]******************\n")
                #     logger.write(str(meta_scores[0]) + "\n")
                ...
                meta_iter = save_new_qa_data(
                    config, valid_questions, RETRIEVAL_SYS, style="gen_rm", answers=valid_answers
                )
                ...
                valid_questions = filter_and_duplicate_removal(filtered_meta_completions)
                meta_iter = save_new_qa_data(config, valid_questions, RETRIEVAL_SYS, style="gen_rm")
        ...
        if meta_iter % 3 == grm_turn:
            if not os.path.exists(f"./exp/{config.trainer.experiment_name}/meta_asl/qa_data_{meta_iter}.jsonl"):
                meta_iter = save_new_qa_data(
                    config, ["This is a placeholder question."], PROMPT_GENERATOR_SYS, style="meta"
                )
        ...
        if os.path.exists(f"./exp/{config.trainer.experiment_name}/meta_asl/qa_data_{meta_iter}.jsonl"):
            ...
            if config.reward_model.online_prompt_generation:
                if meta_iter % 3 == pg_turn:
                    if meta_iter == pg_turn:
                        last_dataset_len = sum(
                            1 for sample in meta_dataset if sample["reward_model"]["style"] == "meta"
                        )
                    elif meta_iter % 3 == pg_turn:
                        last_dataset = load_qa_data(config, meta_iter - 2)
                    ...
                    limit_size = 30000  # last_dataset_len * rollout_n * max(4 - meta_iter // 2, 1)
                    logger.write("******************limit_size******************\n")
                    logger.write(str(limit_size) + "\n")
                    logger.write("******************len(qa_data)******************\n")
                    logger.write(str(len(qa_data)) + "\n")
                    if len(qa_data) > limit_size:
                    ...
                    for i in range(1, len(rewards_history)):
                        if rewards_history[-i][f"{train_type}_scores"] >= 0:
                            recent_scores.append(rewards_history[-i][f"{train_type}_scores"])
                        else:
                            break
                    if len(recent_scores) > recent_nums:
                        ratio_long = (
                            np.mean(recent_scores[: recent_nums // 2])
                            - np.mean(recent_scores[recent_nums // 2 : recent_nums])
                        ) / np.mean(recent_scores[recent_nums // 2 : recent_nums])
                        print(ratio_long)
                        logger.write("******************ratio_long******************\n")
                        logger.write(str(ratio_long) + "\n")
                        ratio_short = (
                            np.mean(recent_scores[: recent_nums // 4])
                            - np.mean(recent_scores[recent_nums // 4 : recent_nums // 2])
                        ) / np.mean(recent_scores[recent_nums // 4 : recent_nums // 2])
                        print(ratio_short)
                        logger.write("******************ratio_short******************\n")
                        logger.write(str(ratio_short) + "\n")
                        if ratio_long < 0.005 and ratio_short < 0.005:
                            new_dataset = datasets.Dataset.from_list(qa_data)
                            logger.write("******************len(new_dataset)******************\n")
                            logger.write(str(len(new_dataset)) + "\n")
                            if meta_iter % 3 == grm_turn:
                                ...
                                mixed_dataset = new_dataset  # meta_dataset
                            mixed_dataset = mixed_dataset.shuffle(seed=37)
                            mixed_dataset.to_parquet(
                                f"./exp/{config.trainer.experiment_name}/meta_asl/qa_data_{meta_iter}_train.parquet"
                            )
                            if meta_iter % 3 == grm_turn:
                                mixed_dataset.select(range(1)).to_parquet(
                                    f"./exp/{config.trainer.experiment_name}/meta_asl/qa_data_{meta_iter}_test.parquet"
                                )
                            open(f"./exp/{config.trainer.experiment_name}/meta_asl/meta_iter.txt", "w").write(
                                str(meta_iter + 1)
                ...
                if len(qa_data) > last_dataset_len * (
                    rollout_n if meta_iter % 2 == 1 else 1
                ):  # this condition should be carefully designed
                    new_dataset = datasets.Dataset.from_list(qa_data)
                    if meta_iter == 1:
                        logger.write("******************len(new_dataset)******************\n")
                        logger.write(str(len(new_dataset)) + "\n")
