...
online_prompt_generation_template_new = """
I asked a question to my students:
{question_prompt}
...
You should use the tool to retrieve the relevant infomation about the question, then generate the new question by {question_constraint} one-hop reasoning steps to the current question.
e.g. {demo}
Output with the following format:
<question>
...a new question...
    ...
    qa_data = [
        {
            ...
            "prompt": [
                {"role": "system", "content": RETRIEVAL_SYS if style == "gen_rm" else PROMPT_GENERATOR_SYS},
                {
                    "role": "user",
                    "content": question,
                },
...
@rewards_registry.register(alias=["asl_reward_depth"], ignore_if_exist=True)
class AgenticSelfLearningRewardDepth(WithWorkerGroupMixin, GRPORewards):
        ...
        **kwargs,
        ...
        if do_gen_rm_verification:
            ...
            if config.reward_model.online_prompt_generation:
                ...
                accuracy_for_questions = [
                    np.mean(
                        [
                            ...
                            if gen_rm_question_prompts[i] == question
                        ]
                    )
                ...
                online_prompt_generation_new_prompts = [
                    online_prompt_generation_template_new.format(
                ...
                meta_iter = save_new_qa_data(
                    config, online_prompt_generation_new_prompts, logger, style="meta", answers=gt_for_questions
                )
        ...
        if do_meta_verification:
            ...
            if config.reward_model.online_prompt_generation:
                ...
                meta_iter = save_new_qa_data(config, valid_questions, logger, style="gen_rm", answers=valid_answers)
        ...
        if os.path.exists(f"./exp/{config.trainer.experiment_name}/meta_asl/qa_data_{meta_iter}.jsonl"):
            ...
            if config.reward_model.online_prompt_generation:
                if meta_iter % 2 == 1:  # this condition should be carefully designed
                    if len(qa_data) > last_dataset_len * (rollout_n if meta_iter == 1 else (rollout_n / 2)):
                    ...
                    recent_gen_rm_scores = []
                    for i in range(1, len(rewards_history)):
                        if rewards_history[-i]["gen_rm_scores"] >= 0:
                            recent_gen_rm_scores.append(rewards_history[-i]["gen_rm_scores"])
                        else:
                            break
                    if len(recent_gen_rm_scores) > 200:
                        ratio = (
                            np.mean(recent_gen_rm_scores[:100]) - np.mean(recent_gen_rm_scores[100:200])
                        ) / np.mean(recent_gen_rm_scores[100:200])
                        print(ratio)
                        logger.write("******************ratio******************\n")
