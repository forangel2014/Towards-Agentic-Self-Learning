...
EVENT_SERVER_NAME = "qs-redaccel-event-default"
...
class WebHookEventSink(EventSink):
    ...
    def send(self, data: dict):
        ...
        if data.get("redaccel_rt_status") != RunStatus.FAIL or self.reported:
            return
    ...
    def _send(self, webhook_url, data: Dict):
        elapsed = data.get("redaccel_rt_elapsed")
        error_message = data.get("redaccel_rt_error_message", "")
...
class EventTracker:
    ...
    _RT_PREFIX = "redaccel_rt_"
    _CFG_PREFIX = "redaccel_cfg_"
    ...
    def _init(self, **init_kwargs):
        ...
        data = {
            ...
            "qs_docker": None,
            "redaccel_version": get_versions()["version"],
            # 在 trial 内可唯一标识即可，防止 int32 溢出
            "redaccel_rt_run_id": int(time.time()) % 114514666,
        }
    ...
    def track_args(self, args: List[Any]):
        ...
        if data.get("redaccel_cfg_use_multi_cloud_unified_ai_dataset") or data.get(
            "redaccel_cfg_use_multi_cloud_unified_ai_dataset_v2"
        ):
            data["redaccel_cfg_use_multi_cloud_ai_dataset"] = True

        # for ds zero
        for arg in args:
            if hasattr(arg, "deepspeed_plugin") and arg.deepspeed_plugin is not None:
                data["redaccel_cfg_ds_zero"] = arg.deepspeed_plugin.zero_stage
                data["redaccel_cfg_ds_offload"] = arg.deepspeed_plugin.offload_optimizer_device != "none"
        self.updates(data)
    ...
    def catch_main(self):
        ...
        try:
            ...
            if train_end_time := self.data.get("redaccel_rt_train_stop_time"):
    ...
    def emit(self, must: bool = True):
        ...
        self.data["redaccel_rt_event_timestamp"] = self.data["_timestamp"]
    ...
    def from_env(cls, command: Optional[str] = None):
        ...
        command = command or os.environ.get("REDACCEL_COMMAND", "train")
        # TODO(wuhuan): 暂时只支持 train 和 rl
        if command not in ("train", "rl"):
            return cls(sinks, redaccel_rt_command=command)
