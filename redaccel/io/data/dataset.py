...
class _RedBaseDataset(Dataset):
    ...
    def get_image(self, image_id):
        ...
        if Configs.REDACCEL_ENABLE_IMAGE_CLEAR:
    ...
    def get_redaccel_imgs(self, data):
    ...
    def get_redaccel_imgs_path(self, data):
        if isinstance(data, dict):
            for key, value_list in data.items():
    ...
    def get_full_sample(self, sample_empty_image, image_ids):
        sample_empty_image["redaccel_imgids"] = copy.deepcopy(image_ids)
        redaccel_imgs = self.get_redaccel_imgs(image_ids)
        sample_empty_image["redaccel_imgs"] = redaccel_imgs
        if self.use_as_dict:
    ...
    def get_full_sample_sharegpt(self, sample_empty_image, image_ids):
        ...
        redaccel_imgs = self.get_redaccel_imgs_path(image_ids)
        sample_empty_image["images"] = redaccel_imgs
        if self.use_as_dict:
            return self.parse_data_fn.run(sample_empty_image)
        else:
            return self.parse_data_fn.run(RedData(sample_empty_image))
    ...
    def parse_redacceljson(self, total_samples):
        assert (
...
class _RedDataset(IterableDataset, _RedBaseDataset):
    ...
    def __iter__(self):
        ...
        for _ in range(self.repetitions):
            try:
                if self.dataset_format in ["sharegpt", "alpaca"]:
                    self.dataset_index = self.parse_redacceljson(len(self))
                else:
